...
class SnapshotTree(object):
    ...
    def selectSnapshot(self, event):
        ...
        item = event.GetItem()
        try:
            nodeData = self._infoTree.GetItemData(item)
    ...
    def update(self, data = None):
        ...
        try: 
            ...
            nodeData = tree.GetItemData( rootItem )
            snapshot = nodeData.getObject().getCurrentSnapshot()
        except AttributeError: 
            return
        
        if self._activeTreeItemId != None :
            currentSnapshot = tree.GetItemData( self._activeTreeItemId )
        ...
        while len(activeList) > 0 :
            ...
            object = tree.GetItemData( treeItemId ).getObject()
