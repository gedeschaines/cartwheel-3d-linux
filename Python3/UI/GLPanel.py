...
class GLPanel(glcanvas.GLCanvas):
    ...
    def processSizeEvent(self, event):
        ...
        if self.IsShown() and self._glContext:
...
def _createGLUIMouseEvent( mouseEvent ):
    ...
    with warnings.catch_warnings():
        ...
        result.leftDown = mouseEvent.LeftIsDown()
        result.middleDown = mouseEvent.MiddleIsDown()
        result.rightDown = mouseEvent.RightIsDown()
        result.x = mouseEvent.GetX()
        result.y = mouseEvent.GetY()
        result.moving = mouseEvent.Moving()
        result.dragging = mouseEvent.Dragging()

        if hasattr(mouseEvent, 'altDown'):
            result.altDown = mouseEvent.AltDown()
        if hasattr(mouseEvent, 'controlDown'):
            result.controlDown = mouseEvent.ControlDown()
        if hasattr(mouseEvent, 'metaDown'):
            result.metaDown = mouseEvent.MetaDown()
        if hasattr(mouseEvent, 'shiftDown'):
            result.shiftDown = mouseEvent.ShiftDown()

        if hasattr(mouseEvent, 'WheelRotation'):
            result.wheelRotation = mouseEvent.GetWheelRotation()
        if hasattr(mouseEvent, 'WheelDelta'):
            result.wheelDelta = mouseEvent.GetWheelDelta()
        if hasattr(mouseEvent, 'LinesPerAction'):
            result.linesPerAction = mouseEvent.GetLinesPerAction()
