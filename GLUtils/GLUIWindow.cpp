...
GLUISize GLUIWindow::getMinSize() const {
	...
	realMinSize.width = std::max( realMinSize.width, minSize.width );
	realMinSize.height = std::max( realMinSize.height, minSize.height );
	return realMinSize;
}
...
bool GLUIWindow::beginDraw(int offsetX, int offsetY, int maxWidth, int maxHeight, int width, int height) const {
	...
	maxWidth  = std::min( maxWidth,  rect.x + rect.width  - subWindowX );
	maxHeight = std::min( maxHeight, rect.y + rect.height - subWindowY );
