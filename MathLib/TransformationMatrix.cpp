...
TransformationMatrix& TransformationMatrix::operator = (const Matrix& other){
	...
	if (other.getColumnCount()!=4 || other.getRowCount()!=4)
		throwError( "COPY OPERATOR: Matrix passed in did not have the correct dimensions!");
...
void TransformationMatrix::getValues(double* values) const{
	...
	if (this->matrix->tda == 4)
		memcpy(values, this->matrix->data, 16*sizeof(double));
	else{
